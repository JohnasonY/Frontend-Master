...
public:
  ...
  Chain<T> &Extend(const Chain<T> &B);
...
Chain<T> &Chain<T>::Extend(const Chain<T> &B)
{
  ...
  if (aTail)
  {
    // a is not an empty linked list
    // traverse A linked list until the end
    while (aTail->next)
    {
      aTail = aTail->next;
    }
    // aTail points to the last node in the linked list
  }

  Node<T> *curBnode = B.first;
  while (curBnode)
  {
    Node<T> *newNode = new Node<T>;
    newNode->data = curBnode->data;
    newNode->next = 0;
